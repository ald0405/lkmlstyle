...
def all_rules():
    return {
        k: v for k, v in RULES_BY_CODE.items() if k in ("M106", "D102", "M110", "V100")
    }
...
def test_ignore_should_exclude_rules(all_rules):
    ...
    chosen = choose_rules(all_rules, ignore=("M106", "D102"))
    assert "M106" not in rules_to_codes(chosen)
...
def test_select_should_exclude_rules(all_rules):
    ...
    chosen = choose_rules(all_rules, select=("V100", "D102"))
    assert "V100" in rules_to_codes(chosen)
    assert "D102" in rules_to_codes(chosen)
